...
def main(args):
  ...
  fin = csv.reader(open(args._in,'r'))
  ...
  # qc columns
  qc_cols = ['31-0.0', # sex, self-reported
      '21000-0.0', # ethnicity, self-reported
      '22001-0.0', # sex, genetic
      '22009-0.1','22009-0.2', # first two genetic PCs
      '22027-0.0'] # excessive heterozygosity
  qc_col_ids = []
  ...
  if len(error_list) > 0:
    ...
    for j in error_list:
      print(pheno[j], file = flog); print(pheno[j])
    print('\n', file = flog)
  ...
  for line in fin:
    ...
    if not args.noqc:
      ...
      if not line[qc_col_ids[5]] in ['NA','na',None, '']: het += 1; continue # heterozygosity, must be NA
...
if __name__ == '__main__':
  ...
  parser.add_argument('-i','--in', dest = '_in', help = 'input ukb fetch file, CSV format',
    default = '/rds/project/rb643/rds-rb643-ukbiobank2/Data_Phenotype/DataFetch_20022024/ukb677594.csv')
    # default = '/rds/project/rb643/rds-rb643-ukbiobank2/Data_Phenotype/DataFetch_09052025/ukb679961.csv')
