...
snps = pd.read_table(f'{prefix}/{prefix}.esi', header = None)
for chrom in snps[0].unique():
    snps.loc[snps[0]==chrom, 1].to_csv(f'temp_snplist_{prefix}/chr{chrom}.txt', header = False, index = False)
    os.system(f'../toolbox/smr --beqtl-summary {prefix}/{prefix} --extract-snp temp_snplist_{prefix}/chr{chrom}.txt --make-besd --out {prefix}/{prefix}_chr{chrom}')
